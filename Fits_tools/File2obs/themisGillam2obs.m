function obs = themisGillam2obs(filename,SeqNrOfImg,varargin)
% ANYTHING2OBS - Meta-data Obs-struct from filename
...
if ~isfield(obs,'dt')
  ...
  dt = obs.dt;
  % Decimal hours:
  decimal_hours = obs.time(end-2:end)*[1 1/60 1/3600]' + dt*(N1-1)/3600;
  % Integer Hours
...
obs.longlat = [265.34 56.35];% deg GLat, 265.34 deg GLong
obs.alpha = [];
obs.beta = [];
obs.az = [0];
obs.ze = [0];
