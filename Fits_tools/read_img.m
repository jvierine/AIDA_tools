...
switch flt
 case 'fits'
  ...
  if isfield(PREPRO_OPS,'frames')
    ...
    obs.time(6) = obs.time(6) + obs.interval_t*(PREPRO_OPS.frames(1)-1);
 ...
 case 'sbig' % For reading files in SBIG format. Almost fits.
  [~,img_out,img_head] = sbig(filename);
 ...
 case 'miracle_asc' % used to read MIRACLE ASC images
  ...
  [img_out,h,obs]= read_miracle(filename);
  img_head=h;
