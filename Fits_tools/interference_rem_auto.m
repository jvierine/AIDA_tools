...
%              for which fft2(IMG_IN)./fft2(wiener2(IMG_IN,WPSM)) >
...
w = 1;
% A weighted version of the interference patter that should minimize
% the variance of the filtered image, according to Gonzalez and
% Woods.
...
%         blkproc(fftwd,blksiz*[1 1],'mean(x(:))').^2);
  w = ( blockproc(img_in.*fftwd,blksiz*[1 1],'mean(x(:))') - ...
        blockproc(img_in,blksiz*[1 1],'mean(x(:))').*...
        blockproc(fftwd,blksiz*[1 1],'mean(x(:))') )./ ...
      ( blockproc(fftwd,blksiz*[1 1],'mean(x(:).^2)') - ...
        blockproc(fftwd,blksiz*[1 1],'mean(x(:))').^2);
