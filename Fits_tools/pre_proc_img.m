...
if ~isempty(obs.camnr) && obs.camnr == 6
...
if ~isempty(obs.camnr) && obs.camnr == 6 && manual_ccd6
...
if ~isempty(PREPRO_OPS.remove_these_stars)
  ...
  if isstruct(optpar)
    [ua,wa] = project_directions(az',ze',optpar,optpar.mod,bxy-[0,2]*stripsize);
  else
    [ua,wa] = project_directions(az',ze',optpar,optpar(9),bxy-[0,2]*stripsize);
  end
...
%                         fun);
      img_out = blockproc(img_out,size(img_out)./PREPRO_OPS.outimgsize, ...
                        fun);
...
if ( isfield(PREPRO_OPS,'to_per_second') && ...
     PREPRO_OPS.to_per_second && ...
     isfield(obs,'exptime') && ...
     ~isempty(obs.exptime) )
  img_out = img_out/obs.exptime;
end
