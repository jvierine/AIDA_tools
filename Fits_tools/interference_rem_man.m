function img_out = interference_rem_man(img_in,type,method,nr_readout_regs)
...
%                 blkproc(Cm,blksiz*[1 1],'mean(x(:))').^2);
          w = ( blockproc(cm.*Cm,blksiz*[1 1],'mean(x(:))') - ...
                blockproc(cm,blksiz*[1 1],'mean(x(:))').*...
                blockproc(Cm,blksiz*[1 1],'mean(x(:))') )./ ...
              ( blockproc(Cm,blksiz*[1 1],'mean(x(:).^2)') - ...
                blockproc(Cm,blksiz*[1 1],'mean(x(:))').^2);
...
%                 blkproc(Cm,blksiz*[1 1],'mean(x(:))').^2);
          ...
          w = 1;
          warning('could not find: blockproc')
        end
       otherwise
        w = 1;
      end
