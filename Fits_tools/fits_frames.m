...
try
  ...
  for i1 = 1:size(info.PrimaryData.Keywords,1)
    qwe = strfind(info.PrimaryData.Keywords(i1,1),'BITPIX');
    if ~isempty(qwe{1})
      break
    end
  end
  BP = info.PrimaryData.Keywords{i1,2}
end
% BP = [8 16 16 ];
if ( nargout > 0 )
  ...
  for i1 = length(out_frames):-1:1
    ...
    FullFrameExist = fseek(fid,f_position+abs(BP/8)*(out_frames(i1))*sx*sy,'bof');
    if FullFrameExist == 0
      ...
      status = fseek(fid,f_position+abs(BP/8)*(out_frames(i1)-1)*sx*sy,'bof');
