...
\date{Draft for AIDA Browser Calibrator v0.2.4}
...
The lucky workflow runs several bounded stages. Early stages use bright stars
and conservative triangle counts. If no seed is found, later stages allow more
detections and fainter stars. After a seed exists, projected matching is used
to add stars near the current model. For \texttt{optmod 2}, the current code
uses a restricted all-sky style pre-flattening grid that was found to work well
for allsky7 fisheye images:
\begin{equation}
  f_1 \in \{0.80,0.70,0.90,1.00,0.60\},
  \qquad
  a_r \in \{0.90,0.75,1.05,0.60\}
\end{equation}
in the first bright-star bootstrap. Deeper stages widen these ranges and add
more detections. This staged search is a compromise between robustness and
