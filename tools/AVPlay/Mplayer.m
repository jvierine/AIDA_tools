...
%
% Mplayer(...,'currentFrameIndex', [1, numberOfFramesInFile])
%   Optionally specify a non-default index for starting playback. (Default:
%   1).
%
% Mplayer(...,'measureFrameRate', TF)
...
%
% Mplayer(..., 'numberOfFramesToPlay',numberOfFramesToPlay)
...
%
% Mplayer(..., 'showSoundAndProgress',TF)
...
%   during playback.
%
% Mplayer(..., 'singleton', TF)
%   'singleton' (= true/false; Default: true) indicates whether existing
%   Mplayer instances should be automatically closed first.
%
% Mplayer(..., 'throttleToVideoFrameRate', TF)
...
%   a priori, great! Otherwise, Mplayer will figure it out for you. (But it
%   may take a moment.)
%
% Mplayer(..., 'videoProcessFcn', fcn)
...
%
% [vidReader,audioInformation] = Mplayer(...)
...
%
% Mplayer('xylophone.mpg')
%
% % (If timestamp vector was previously saved in 'xylophoneTimestamps.mat'):
% Mplayer('xylophone.mpg', 'timeStamps', 'xylophoneTimestamps.mat')
%
...
%
% %(FrameSize: 240x416x3)
% Mplayer('BigBuckBunny_512kb.mp4',...
%   'playerColor', [0 0 0.5]);
%
...
if nargin < 1
  ...
  if ~ischar(fName) % ~isstr(varargin{1})
    % tmpname = deblank(['/tmp/mplayer',num2str(round(1e8*rand(1,1)),'%08d'),'.avi']);
    try
      tmpname = fullfile('/tmp',['mplayer',num2str(round(1e8*rand(1,1)),'%08d')],'.avi');
      vidObj = VideoWriter(tmpname);
      open(vidObj)
      for iFrame = 1:numel(fName)
        writeVideo(vidObj,fName(iFrame))
      end
      close(vidObj)
      fName = tmpname;
      AVP.fName = fName;
      ...
      % I don't know how to write tmp-files unless on a file-system where
      % there is a /tmp directory
      disp(['Could not create temporary avi-file: ',tmpname])
      error('Error in Mplayer')
    end
  else
    AVP.fName = fName;
    AVP.removefile = 0;
...
% NESTED SUBFUNCTIONS:
    ...
    function jumpTo(~,~,request,requestedValue)
        ...
        switch request
            ...
            otherwise
                error('Mplayer/jumpTo: Unrecognized ''method'' string.')
        end
    ...
    function setupMplayererEnvironment(varargin)
        % Set up environment
        if AVP.singleton
            delete(findobj('tag','myMplayerer'));
        end
