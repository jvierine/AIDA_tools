...
%
% Modified by Bjorn Gustavsson 2006/03/03
...
if nargin == 0 || nargin == 1
  %Initialize
  delete(findobj('tag','mplayer'));
  if nargin == 1
    if ~ischar(varargin{1}) % ~isstr(varargin{1})
      tmpname = deblank(['/tmp/mplayer',num2str(round(1e8*rand(1,1)),'%08d'),'.avi'])
      % movie2avi(varargin{1},tmpname);
      vidObj = VideoWriter(tmpname);
      open(vidObj)
      for iFrame = 1:numel(varargin{1})
        writeVideo(vidObj,varargin{1}(iFrame))
      end
      close(vidObj)
      [pathname, filename, tmp1] = fileparts(tmpname);
      playvals.tmpfilename = tmpname;
    else
...
% $$$          'position', [0.25 0.2 0.5 0.55],...
% $$$          'name',['Sequence: ' fullfile(pathname, filename)],...
% $$$          'NumberTitle', 'off',...
% $$$          'doublebuffer', 'on', ...
% $$$          'backingstore', 'off',...
% $$$          'menubar', 'none',...
...
if nargin < 4
  ...
  uicontrol('style','frame',...
            'units','normalized',...
            'position',[0.05 0.01 0.9 0.1925],...
            'foregroundcolor','b',...
            'backgroundcolor',[0.6 0.6 0.6]);
  playvals.playbutton = uicontrol('style', 'pushbutton',...
                                  'string', '>',...
                                  'units', 'normalized',...
                                  'position', [0.075 0.15 0.275 0.04], ...
                                  'callback','feval(getappdata(gcf,''playbuttonfcnhandle''));' , ...
                                  'fontunits', 'normalized',...
                                  'fontsize', 0.8,...
                                  'tag', 'playbutton', ...
                                  'tooltipstring', 'Starts playback and (re)sets increment to 1.');
  playvals.pausebutton = uicontrol('style', 'pushbutton',...
                                   'string', '||',...
                                   'units', 'normalized',...
                                   'position', [0.35 0.15 0.275 0.04], ...
                                   'callback', 'feval(getappdata(gcf,''pausebuttonfcnhandle''));', ...
                                   'fontunits', 'normalized',...
                                   'fontsize', 0.8, ...
                                   'tooltipstring', 'Pauses/un-pauses playback.','tag','pausebutton');
  playvals.rewindbutton = uicontrol('style', 'pushbutton',...
                                    'string', '<<',...
                                    'units', 'normalized',...
                                    'position', [0.2 0.11 0.15 0.04], ...
  ...
  playvals.forwardbutton = uicontrol('style', 'pushbutton',...
                                     'string', '>>',...
                                     'units', 'normalized',...
                                     'position', [0.35 0.11 0.15 0.04], ...
                                     'callback', 'feval(getappdata(gcf,''forwardbuttonfcnhandle''));', ...
                                     'fontunits', 'normalized',...
                                     'fontsize', 0.8, ...
                                     'tooltipstring', 'Increase playback speed. (Speeds playback by skipping frames.)',...
                                     'interruptible','off');
  playvals.forward100button = uicontrol('style', 'pushbutton',...
                                        'string', '+100',...
                                        'units', 'normalized', ...
                                        'position', [0.5 0.11 0.135 0.04], ...
                                        'callback', 'feval(getappdata(gcf,''forward100buttonfcnhandle''));', ...
                                        'fontunits', 'normalized',...
                                        'fontsize', 0.6, ...
                                        'tooltipstring', 'Jump forward 100 frames (or to end of file).');
  playvals.rewind100button = uicontrol('style', 'pushbutton',...
                                       'string', '-100',...
                                       'units', 'normalized', ...
                                       'position', [0.075 0.11 0.135 0.04], ...
                                       'callback', 'feval(getappdata(gcf,''rewind100buttonfcnhandle''));', ...
                                       'fontunits', 'normalized',...
                                       'fontsize', 0.6, ...
                                       'tooltipstring', 'Jump backward 100 frames (or to beginning of file).');
  playvals.restartbutton = uicontrol('style', 'pushbutton',...
                                     'string', 'restart',...
                                     'units', 'normalized',...
                                     'position', [0.075 0.07 0.135 0.04], ...
                                     'callback', 'feval(getappdata(gcf,''restartbuttonfcnhandle''));', ...
                                     'fontunits', 'normalized',...
                                     'fontsize', 0.8, ...
                                     'tooltipstring', 'Return to first frame.','tag','restartbutton');
  playvals.eofbutton = uicontrol('style', 'pushbutton',...
                                 'string', '->end',...
                                 'units', 'normalized',...
                                 'position', [0.5 0.07 0.135 0.04], ...
                                 'callback', 'feval(getappdata(gcf,''eofbuttonfcnhandle''));', ...
                                 'fontunits', 'normalized',...
                                 'fontsize', 0.8, ...
                                 'tooltipstring', 'Go to last frame.','tag','eofbutton');
  playvals.returnbutton = uicontrol('style', 'pushbutton',...
                                    'string', 'end/quit/close',...
                                    'units', 'normalized',...
                                    'position', [0.2 0.07 0.3 0.04], ...
                                    'callback', 'feval(getappdata(gcf,''returnbuttonfcnhandle''));', ...
                                    'fontunits', 'normalized',...
                                    'fontsize', 0.8, ...
                                    'tooltipstring', 'End playback and close AVI Player.',...
                                    'tag','returnbutton');
  playvals.frameslider = uicontrol('style','slider',...
                                   'units','normalized',...
                                   'position',[0.075 0.02 0.55 0.04],...
                                   'min',1,...
                                   'max',playvals.numims,...
                                   'value',1,...
                                   'fontsize',10,...
                                   'userdata',framedir,...
                                   'callback','iptb = getappdata(gcf,''iptb'');feval(getappdata(gcf,''framesliderfcnhandle''),iptb);',...
                                   'sliderstep',[1/playvals.numims 10/playvals.numims]);
  playvals.loop = uicontrol('style','checkbox',...
                            'units','normalized',...
                            'position',[0.64 0.02 0.15 0.04],...
                            'value',0,...
                            'fontsize',8,...
                            'string','Loop',...
                            'backgroundcolor',[0.6 0.6 0.6],...
  ...
  uicontrol('style','frame',...
            'units','normalized',...
            'position',[0.64 0.07 0.29 0.12]);
  playvals.pauseslider = uicontrol('style','slider',...
                                   'units','normalized',...
                                   'position',[0.66 0.11 0.25 0.0375],...
                                   'value',0,...
                                   'min',0,...
                                   'max',5,...
                                   'sliderstep',[0.1, 1/5],...
  ...
  tmp=uicontrol('style','text',...
                'string',sprintf('Inter-Frame Pause (seconds)'),...
                'units','normalized',...
  ...
  uicontrol('style','text',...
            'string',0,...
            'units','normalized',...
            'position',[0.66 0.077 0.02 0.03]);
  uicontrol('style','text',...
            'string',5,...
            'units','normalized',...
            'position',[0.88 0.077 0.02 0.03]);
  playvals.pausesecsbox = uicontrol('style','text',...
                                    'units','normalized',...
                                    'position',[0.765 0.078 0.04 0.0275],...
                                    'string',0,...
                                    'tag','pausesecsbox',...
                                    'fontsize',8,...
                                    'fontweight','b');%,'backgroundcolor',[0 0.6 0.6]
  ...
  if iptb
    if ~isempty(frame.colormap)
      imshow(ind2rgb(frame.cdata,frame.colormap));
    else
      imshow(frame.cdata);
    end
  else
  ...
  playvals.paused = 0;
  playvals.quit = 0;
  % TODO: Change from aviread to VideoReader
...
elseif strcmp(tmp.Tag,'Loop')
  ...
  drawnow;
  mplayer(playvals,framedir,'Continue');
else
  mplayer(playvals,framedir,'Paused');
end
...
while playvals.playcontinue
  ...
  if ~isempty(findobj('tag','mplayer'))
    setappdata(gcf, 'playvals', playvals);
    pauselength = get(playvals.pauseslider,'value');
    %pause(pauselength);
...
playvals.playcontinue = 0;
playvals.quit = 1;
if isfield(playvals,'tmpfilename')
  tmpfilename = playvals.tmpfilename;
  unix(['rm ',tmpfilename]);
end
setappdata(gcf, 'playvals', playvals);
return
