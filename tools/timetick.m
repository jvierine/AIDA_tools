...
if nargin==0
 x='x';
elseif strcmp(x,'off')
 set(curr_ax,'xtickmode','auto','xticklabelmode','auto')
 return
end
tick=[x 'tick']; xt=get(curr_ax,tick); % Not needed/BG 20190327: dx=xt(2)-xt(1); nxt=length(xt);
xl=get(curr_ax,[x 'lim']);

axsiz = get(curr_ax,'position');
if strcmp(x,'x')
...
set(curr_ax,tick,xt,[tick 'mode'],'man')
...
set(curr_ax,[tick 'label'],nt,[tick 'labelmode'],'man')
