...
if strcmp(Uname,'cyrils') % Yup that's my Soton name...
  homedir = '/bira-iasb/home/cyrils/';
else
  [q1,q2,q3] = fileparts(which('AIDA_startup.m'))
  homedir = q1;
  % Or hardcode it:
  % homedir = '/home/bjorn/';
end
...
if strcmp(Uname,'cyrils') % and I "don't like it very much"...
  data_dir = fullfile(homedir,'projects/ALIS/stdnames/');
else
  data_dir = fullfile(homedir,'Example-data');
end
cd(data_dir);
...
%file_list = str2mat(dFiles{1}(12,:),dFiles{2}(12,:),dFiles{3}(12,:),dFiles{4}(12,:),dFiles{5}(12,:),dFiles{6}(12,:));

%%## Changed here too...
if strcmp(Uname,'cyrils')
  ii = 5; 
  file_list = str2mat(dFiles{1}(ii,:),dFiles{2}(ii,:),dFiles{3}(ii,:),dFiles{6}(ii,:));
  stns = tomo_inp_images(file_list,[],POs([1 2 3 6]));
else
  ii = 1; 
  file_list = str2mat(dFiles{1}(ii,:),dFiles{2}(ii,:),dFiles{3}(ii,:));
...
X3D = r0(1)+dr1(1)*(X-1)+dr2(1)*(Y-1)+dr3(1)*(Z-1); % X3D(:,43,:) at EISCAT     , 2.5 km resol
Y3D = r0(2)+dr1(2)*(X-1)+dr2(2)*(Y-1)+dr3(2)*(Z-1); % Y3D(70,:,10) at 100 km alt, 2.5 km resol
Z3D = r0(3)+dr1(3)*(X-1)+dr2(3)*(Y-1)+dr3(3)*(Z-1); % Z3D(1,1,10) 100 km height , 2.5 km resol
...
% Vem0 = aeronomic_alt4tomo(Par,stns(1),Am,Energy,X3D,Y3D,Z3D,tomo_ops);
...
% X3D(:,i_x(...),:) - horizontal distance east of Kiruna in km, 2.5 km
%                     resolution. X3D(i_y,i_x,I_z) 
%                     ex. X3D(:,43,:) = plane X situated -47 km from
%                     Kiruna, containing EISCAT Tromso
% Y3D(i_y(...),:,:) - distance north of Kiruna in km (since we are working
%                     with a 3D-block skewed to be parallel to the magnetic
%                     zenith, each layer is translated to dl * tan(mag_Ze)
%                     to the south, 2.5 km resolution. Y3D(i_y,i_x,I_z)
%                     ex. Y3D(70,:,1) = plane Y situated 140-180 km away
%                     from Kiruna, containing EISCAT Tromso
% Z3D(:,:,i_z(3))   - altitude of projection in km, 2.5 km resolution
...
%%%%%NewGACT_snippetEND
% [Vem,I2D,IeE2D] = tomo_start_guessGACT(stns,Energy,Ie2H,Xslice,Yslice,Zslice,M2Dto1D,U,V,I_cuts,iPeaks,X3D,Y3D,Z3D,ops);

ops4tgact = tomo_start_guessGACT
ops4tgact.normregs = {[],tomo_ops(2).renorm}

[Vem,I2D,IeE2D] = tomo_start_guessGACT(stns([1,2]),Energy,Ie2H,X12,Y12,Z12,M2Dto1D_12,U12,V12,I_cuts,iPeaks,X3D,Y3D,Z3D,ops4tgact);

%%## This should produce something nice here!:
clf
tomo_slice_i(X3D,Y3D,Z3D,Vem,32,65,12),shading flat
StartGuess.Vem = Vem; % Keep the start guess aside -%%## add indices to this one...
StartGuess.IeE2D = IeE2D; % Keep the start guess aside -%%## add indices to this one...
%% 5.5 Reconstructions
% 
