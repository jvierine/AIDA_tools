...
%
% INPUTS:
%   files    - struct with ALIS image files as returned from dir,
%              or a char array with filenames [n_files x max_length_of_files]
%              Note that dir only gives the filenames without the
%              path. filenames with different lengths will result
%              in shorter names being padded with white-spaces,
%              this is handled with STRTRIM in this function.
...
for num = 1:size(files,1)
  ...
  if isstruct(files)
    [im,~,o] = inimg(files(num).name,PO);
  else
    [im,~,o] = inimg(strtrim(files(num,:)),PO);
  end
  filtID(iI) = o.filter;
  ...
  if isempty(dOPS.histmaxint)
    cImg = ASK_bytscl(im,Int_perc(iI,1),Int_perc(iI,2),1);
  else
  ...
  if dOPS.imdisplay
    ...
    % Decorations:
    title(['ALIS ',o.site,'(',sprintf('%d',o.filter),'): ',datestr(o.time,'yyyy-mm-dd HH:MM:SS.FFF')],'fontsize',16);
