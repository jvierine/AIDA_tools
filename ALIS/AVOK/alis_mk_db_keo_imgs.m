function ok = alis_mk_db_keo_imgs()
...
if isunix
  ...
  % Todo: sheck what the heck happens with this! /BG20230623
  rootD = 'c:';
end
Keo = [];
Wl_emi = [];
...
ALIS_data_dirs = '/alis/overviews';

cd(ALIS_data_dirs)

[~,overview_files] = my_unix('ls -1 alis_overview[0-9]*.mat');
...
for i_day = 1:length(overview_dates)
  ...
  for i2 = length(data_years):-1:1
    ...
    for i3 = length(data_month):-1:1
      ...
      for i4 = length(data_days):-1:1
        ...
        savefile = fullfile(rootD,'alis','overviews','alis_overview',[todaystr,'New','.mat']);
        if ~exist(savefile,'file')
          ...
          for i5 = 1:length(stn_keys)
            ...
            if ~isempty(w)
              ...
              todaystr = [data_years(i2).name,data_month(i3).name,data_days(i4).name];
              % TODO: Check that this does what's intended. /BG20230623
              savestr = ['save /alis/overviews/alis_overview',todaystr,' Keo Wl_emi T_obs Filenames Optps Stns'];
              % eval(savestr)
              % save(savefile,'Keo','Wl_emi','T_obs','Filenames','Optps','Stns')
            end
          end
