function avok = alis_view_overview_keos(date)
...
if isunix
  [~,w] = my_unix(['find ',AvOk.data_inputdir,' -name \*[0-9]\*.mat']);
  AvOk.overview_files = w(1:end-1,:);
else
  % TODO: Check that this actually works. /BG20230623
  Cw = dir2(AvOk.data_inputdir,'-r','*.mat');
  Cw = char(Cw.name);
  w = nan(size(Cw,1),size(AvOk.data_inputdir,2)+size(Cw,2)+1);
  w = char(w);
  for i1 = 1:size(w,1)
    w(i1,:) = fullfile(AvOk.data_inputdir,Cw(i1,:));
  end
  AvOk.overview_files = w;
end
  ...
  for i = 1:length(ufnr)
    ...
    if any(wl_emi==ufnr(i))
      ...
      if sum(wl_emi==ufnr(i))>1
        if numel(keo(:,wl_emi==ufnr(i))) > 800
