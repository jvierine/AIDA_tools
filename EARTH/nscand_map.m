function PH = nscand_map(latlong_or_xyz,axlim,rCshift)
% NSCAND_MAP - plot topographic map over northern Scandinavia
%   
% Calling:
...
if nargin < 3 || isempty(rCshift)
  rCshift = [0 0 0];
end
%Was: if nargin & strcmp(lower(latlong_or_xyz(1)),'l')
if nargin && ~isempty(latlong_or_xyz) && strcmpi(latlong_or_xyz(1),'l')
  ...
  if nargin == 2 && ~isempty(axlim)
    ...
    ph = tcolor(xNscand2p-rCshift(1),yNscand2p-rCshift(2),Nscand2p);shading flat
  else
    ph = tcolor(xNscand-rCshift(1),yNscand-rCshift(2),Nscand);shading flat
  end
end
if nargout
  PH = ph;
