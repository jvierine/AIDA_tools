...
%% Old variant of setting the path to AIDA_tools
...
%% New variant of setting the path to AIDA_tools
[AIDA_matlabdir,S2,S3] = fileparts(which('AIDA_startup'));
% For those that work under non-unix OSes I think that this should work. If
% AIDA_matlabdir does not look like the path of the AIDA_tools directory,
...
if ismac
  % To interface with the Pulkkovo spectrophotometric catalog a
  % csh or bash-script needs to be run, that bash-script calls gawk
  % to parse and write data. On the mac-computer this was tested on
  % gawk got installed into /opt/local/bin. That directory was not
  % on the search-path in the system-calls from inside
  % matlab. These couple of lines seems to add that directory to
  % the path and solve the issue.
  [~,envPATH] = system('echo -n $PATH');
  envPATH = [envPATH,':/opt/local/bin:/opt/local/sbin'];
  setenv('PATH',envPATH)
  % How to do this for different windows-systems I do not know.
end
% Determine which matlab version that is running
M_v = ver('matlab');
M_ver = min(9,floor(str2num(M_v.Version)));
...
path(path,fullfile(AIDA_matlabdir,'.data','Political_boundaries'))
...
path(path,fullfile(AIDA_matlabdir,'Skymap','stars'))
...
path(path,fullfile(AIDA_matlabdir,'tools','AVPlay'))
...
path(path,fullfile(AIDA_matlabdir,'.data','Test-imgs'))
path(path,fullfile(AIDA_matlabdir,'Example-data'))
