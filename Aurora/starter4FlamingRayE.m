...
for i1 = 1:size(ImStack{1},3)
  img4bg = wiener2(medfilt2(ImStack{1}(:,:,i1),[3,3],'symmetric'),[3,3]);
  img4bg(bgMask==1) = nan;
  imbg{1}(:,:,i1) = inpaint_nans(img4bg,4);
  img4bg = wiener2(medfilt2(ImStack{3}(:,:,i1),[3,3],'symmetric'),[3,3]);
  img4bg(bgMask==1) = nan;
  imbg{3}(:,:,i1) = inpaint_nans(img4bg,4);
end

for i1 = 1:size(ImStack{1},3)
  imbg3{1}(:,:,i1) = imbg{1}(:,:,max(1,i1-1))/4+imbg{1}(:,:,i1)/2 + imbg{1}(:,:,min(i1,size(ImStack{3},3)))/4;
  imbg3{3}(:,:,i1) = imbg{3}(:,:,max(1,i1-1))/4+imbg{3}(:,:,i1)/2 + imbg{3}(:,:,min(i1,size(ImStack{3},3)))/4;
end

%% 2b Calculate as good a back-ground as possible for each image in
%  6730 and 7774. The inpaint_nans function provides several fancy
%  inpainting (interpolating into areas from values on the
%  boundary, possibly taking into account gradients too):
bgMask2 = bgMask;
bgMask2(188:226,256) = 0;
iY = [[37:42]+149,220:230];
for i1 = 20:-1:1
  cIedge = wiener1(squeeze(ImStack{1}(:,end,i1))',3);
  [pp,S] = polyfit(iY,cIedge(iY),1);
  [BG1(i1,:),dBG4(i1,:)] = polyval(pp,150:256,S);
end

for i1 = 1:size(ImStack{1},3)
  ImStack{1}(1,:,i1) = ImStack{1}(2,:,i1);
  ImStack{1}(:,1,i1) = ImStack{1}(:,2,i1);
  ImStack{1}(end,:,i1) = ImStack{1}(end-1,:,i1);
  ImStack{1}(:,end,i1) = ImStack{1}(:,end-1,i1);
  ImStack{3}(1,:,i1) = ImStack{3}(2,:,i1);
  ImStack{3}(:,1,i1) = ImStack{3}(:,2,i1);
  ImStack{3}(end,:,i1) = ImStack{3}(end-1,:,i1);
  ImStack{3}(:,end,i1) = ImStack{3}(:,end-1,i1);
end
for i1 = 1:size(ImStack{1},3)
  ...
  img2bg = wiener2(medfilt2(ImStack{1}(:,:,i1),[3,3],'symmetric'),[3,3]);
  img2bg(bgMask2==1) = nan;
  im2bg(150:256,256) = BG1(i1,:);
  imbg2{1}(:,:,i1) = inpaint_nans(img2bg,4);
  
  cIedge = wiener1(squeeze(ImStack{1}(:,end,i1))',3);
  [pp,S] = polyfit(iY,cIedge(iY),1);
  [BG1(i1,:),dBG4(i1,:)] = polyval(pp,150:256,S);
  
  img2bg = wiener2(medfilt2(ImStack{3}(:,:,i1),[3,3],'symmetric'),[3,3]);
  img2bg(bgMask==1) = nan;
  im2bg(150:256,256) = BG1(i1,:);
  imbg2{3}(:,:,i1) = inpaint_nans(img2bg,4);
end
...
%            I0        x0     dx           y0      dy       g_x     E0      dE      g_E     g_E2  phiE   I1      E0      dE        g_E     g_E2                  
parMem  = {   'I0',     'x0',    'dx',    'y0',    'dy',  'g_x',   'E0',   'dE',  'g_E',  'g_E2','phiE',   'I1',   'E0',     'dE',  'g_E','g_E2'}                  
...
stns(2).img = Iq;
parI = fminsearchbnd(@(I) err4FlamingRaysE(I,vpI,I0b,stns,{bgMask,bgMask},ZfI,XfI(:,:,115),YfI(:,:,115),{A1Z,A2Z},E,1,113,errOps),parI,[eps(1) eps(1)],[1e6 1e6],fmsOPS);
%%
