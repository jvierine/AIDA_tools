function [stns,XfI,YfI,ZfI] = projection_setup3DtoASK123(optpASK,ds,n_xyz)
% PROJECTION_SETUP3DTOASK123 - 
%   


%% 1 Input parameter handling
if nargin < 1 || isempty(optpASK)
  %% 1.1 Load default ASK optical parameters
...
if nargin < 2 || isempty(ds)
  %% 1.2 Set up the geometry of the base function block
  ds = 1/6;  % Size (full-width-at-half-max) of base function in km:
  ds = 1/4.5;
else
  % No need to do anything else!
end
if nargin < 3 || isempty(n_xyz)
  ...
  sx = round(68*4.5/6)*2 % Number of base functions in WEST-EAST direction
  sy = round(68*4.5/6)*2 % Number of base functions in NORTH-SOUTH direction
...
ze_B = 7.7023*pi/180;   % Modified for ESR 2016/BG20250502, was: (90-82.28)*pi/180;   % dip angle average 100<-300
az_B = 187.4323*pi/180; % Modified for ESR 2016/BG20250502, was: (180+4.05)*pi/180; % azimuth angle (clockwise from north)
...
drawnow
r0_top = [ax([1,3]),h0];
r0 = point_on_line(r0_top,e_B,-100)+[0,2,0];
...
I3D0(col_idx(1),row_idx(1),:) = 1.2;
I3D0(col_idx(1),row_idx(2),:) = 1.2;
I3D0(col_idx(1),row_idx(3),:) = 1.2;
I3D0(col_idx(1),row_idx(4),:) = 1.2;
I3D0(col_idx(2),row_idx(1),:) = 1.2;
I3D0(col_idx(2),row_idx(2),:) = 1.2;
I3D0(col_idx(2),row_idx(3),:) = 1.2;
I3D0(col_idx(2),row_idx(4),:) = 1.2;

I3D0(col_idx(1),row_idx(1),z_idx) = 110;
I3D0(col_idx(1),row_idx(2),z_idx) = 120;
I3D0(col_idx(1),row_idx(3),z_idx) = 130;
I3D0(col_idx(1),row_idx(4),z_idx) = 130;
I3D0(col_idx(2),row_idx(1),z_idx) = 120;
I3D0(col_idx(2),row_idx(2),z_idx) = 120;
I3D0(col_idx(2),row_idx(3),z_idx) = 120;
I3D0(col_idx(2),row_idx(4),z_idx) = 120;
