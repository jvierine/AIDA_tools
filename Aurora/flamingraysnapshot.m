...
subplot(4,1,1)
pcolor([1:20]*1/32,E*1e3,log10(IeRay'*1e6))
shading flat
caxis([-5 0]+max(caxis))
axIe = axis;
axis([axIe(1:3),5e3])
...
for i1 = idxs2plot
  ...
  ph = plot((i1+0.5)/32*[1 1],E([1,end])*1e3,'k','linewidth',2);
  subplot(4,2,5)
  imagesc(res.currImg{1}),imgs_smart_caxis(0.0003,res.currImg{1}),axis([150 256 150 256]) 
  hold on
  [~,hC1] = contour(res.currProj{1},5,'w');set(hC1,'linecolor',[1,1,1]*0.99)
  ...
  hold on
  [~,hC2] = contour(res.currProj{2},5,'w');set(hC2,'linecolor',[1,1,1]*0.99)
  plot(xG,yG,'w:','linewidth',2,'color',[1 1 1]*0.99)
  plot(xG',yG','w:','linewidth',2,'color',[1 1 1]*0.99)
  hold off
  set(gca,'xtick',[150:25:250])
  ...
  plot(xG',yG','w:','linewidth',2,'color',[1 1 1]*0.99)
  %grid on
  % cImg = medfilt2(res.currImg{2},[3,3]);
  % cImg = (res.currImg{2}(150:230,:));
  % plot(256-25/max(res.currProj{2}(:))*max(res.currProj{2}),'w')
  % plot(256-50*((max(cImg)-1000)/max(cImg(:)-1000)),'k')
  hold off
  set(gca,'yticklabel','')
  set(gca,'xtick',[150:25:250])
  set(gca,'ytick',[150:25:250])
