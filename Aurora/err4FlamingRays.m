function res = err4FlamingRays(Par,var_pars,par0,stns,ImRois,Z3D,x2D,y2D,Ie2H,E,out_arg_type,z_max,errOps)
...
if ~isempty(Par)
  I0(var_pars) = Par; % Insert the variable parameters into that
                      ...
                      % subspace search...
  I0 = reshape(I0,10,[])'; % Reshape parameter array to [nArcs x 10] to
                          % make for [I0 x0 dx y0 dy gx E0 dE gE g_E2] 
end
...
for i1 = 1:(size(I0,1))
  ...
  % precipitation:
  I_hor = Imax * exp( -abs( ((y2D - y0)./dy).^2 + ...
                            ((x2D - x0)./dx).^2).^(g_x/2));
