function res = err4FlamingRaysAW_I_lambda_tz_given(Par,stns,Imgstacks,ImRois,Z3D,x2D,y2D,I_lambda_of_tz,out_arg_type,z_max,errOps)
...
if ~isempty(Par)
  I0(var_pars) = Par; % Insert the variable parameters into that
                      ...
                      % subspace search...
  I0 = reshape(I0,11,[])'; % Reshape parameter array to [nArcs x 11] to
                           % make for [I0 x0 dx y0 dy gx E0 dE gE g_E2 phiXY] 
end
...
for i1 = 1:(size(I0,1))
  ...
  y0   = I0(i1,4);     % Horizontal center of this arc
  dy   = I0(i1,5);     % Horizontal width of this arc
  g_x  = I0(i1,6);     % Spatial shape factor of this arc -
                       % bias towards 2, constrain between 1.5 and 3
  E0   = I0(i1,7);%*1e3% Central energy of this arc
  dE   = I0(i1,8);%*1e3% Width in energy of this arc
  g_E  = I0(i1,9);     % Shape factor energy-wise of this arc
  g_E2 = I0(i1,10);    % Exponent for power of E before the
  ...
  % Ie = E.^g_E2.*exp(-abs(E-E0).^g_E/dE^g_E).*E.*gradient(E); 
  ...
  % precipitation:
  I_hor = Imax * exp( -abs( ((cos(phiXY)*(y2D - y0)+sin(phiXY)*(x2D - x0))./dy).^2 + ...
                            ((cos(phiXY)*(x2D - x0)-sin(phiXY)*(y2D - y0))./dx).^2).^(g_x/2));
...
if derrOps.bias2cylindrical > 0
  err =   derrOps.bias2cylindrical*sum(((dX(1:end-1)-dY(1:end-1))./(dX(1:end-1)/2+dY(1:end-1)/2)).^2);
else
  err = 0;
end
% derrOps.bias2x0y0 = 0;
% derrOps.x0y0 = [0,0];
if derrOps.bias2x0y0 > 0
  for i1 = 1:size(derrOps.x0y0,1)
    err =   err + derrOps.bias2x0y0*( (derrOps.x0y0(i1,1)-X0(i1))^2 + (derrOps.x0y0(i1,2)-Y0(i1))^2 );
  end
else
  err = 0;
end
