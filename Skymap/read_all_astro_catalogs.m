...
if nargin < 3 || isempty(get_these_fields)
  ...
  for fi = get_these_fields
    ...
    switch field_type(fi)
     case 'A'
      ...
      % eval(str_ini)
      star_list(ii).(deblank(field_name(fi,:)))  = catalogl(field_pos(fi,1):field_pos(fi,2));
     case 'I'
      ...
      % eval(str_ini)
      star_list(ii).(deblank(field_name(fi,:))) = str2num(catalogl(field_pos(fi,1):field_pos(fi,2)));
     case 'F'
      %str_ini = ['star_list(ii).',deblank(field_name(fi,:)),' = str2num(catalog(ii,field_pos(fi,1):field_pos(fi,2)));'];
      %eval(str_ini)
      % str_ini = ['star_list(ii).',deblank(field_name(fi,:)),' = str2num(catalogl(field_pos(fi,1):field_pos(fi,2)));'];
