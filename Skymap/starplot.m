...
if isempty(SkMp.img)
  ...
  hold on
  BSNr_list = [SkMp.star_list.Bright_Star_Nr];
  def_clr = SkMp.prefs.pl_cl_st;
  def_sz = SkMp.prefs.pl_sz_st;
  for i = sz(1):-1:1
    if 180/pi*pstars(i,6) > 0
      f_h = plot(180/pi*pstars(i,6)*cos(-pstars(i,5)),...
                 180/pi*pstars(i,6)*sin(-pstars(i,5)),...
                 '.',...
                 'markersize',max(def_sz,(7-pstars(i,4))^1.5*def_sz),...
                 'color',def_clr);
      i_s = find(BSNr_list == pstars(i,8));
      if SkMp.star_list(i_s).spectra == 1
        % set(f_h,'facecolor',SkMp.star_list(i_s).rgb)
        set(f_h,'color',SkMp.star_list(i_s).rgb)
      end
    end
  end
  pstarsout = 1;
  if isfield(SkMp,'planetary_positions')
