function [possiblestars,catalog] = loadstars3(pos0,date,time0)
% LOADSTARS3 load stars from the: Bright Star Catalogue, 5th Revised
% Ed. (Hoffleit+, 1991) and pick the stars that are above the
% horizon at POS0 (longitude, latitude [deg]) at time TIME0 (UTC)
% on the day DATE.
% 
% Calling:
% [possiblestars,catalog] = loadstars3(pos0,date,time0)
% 
...
[az,ze,~] = starpos2(ra,decl,date,time0,lat,long);
[i_above] = find(ze<pi/2);

possiblestars(:,1:7) = [ra(i_above), decl(i_above), ...
                        az(i_above), ze(i_above), ... 
                        i_above magn(i_above),...
                        Bright_Star_Nr(i_above)];
