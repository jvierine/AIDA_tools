function [lon,lat,alt,xB,yB,zB] = getFieldLines(time_vec,LongLat0,fieldlinelength,OPS)
% getFieldLines - magnetic field lines from the IGRF.
% 
...
dOPS.font = 'Times New Roman';
dOPS.ax_fontsize = 12;
dOPS.label_fontsize = 15;
dOPS.plotit = 0;
if nargin == 0
  lon = dOPS;
  return
...
if nargin < 2 || isempty(LongLat0)
  lat_start = 69.586; % Geodetic latitudes in degrees.
  lon_start = 19.227; % Geodetic longitudes in degrees.
else
  lat_start = LongLat0(2); % Geodetic latitudes in degrees.
  lon_start = LongLat0(1); % Geodetic longitudes in degrees.
end
alt_start = 0; % Altitude in km.

if nargin < 3 || isempty(fieldlinelength)
  distance = -50e3; % km.
else
  distance = fieldlinelength;
end
if lat_start > 0
...
if dOPS.plotit
  ...
  title(['Magnetic Field Line at ' datestr(time)], 'FontName', dOPS.font, ...
        'FontSize', dOPS.label_fontsize, 'Color', 'k');
  set(gca,'fontsize',dOPS.ax_fontsize)
