function [ph,sph] = spc_cal_3Dplot_star(starID,plotPar,IDSTARS,OPTS)
...
for iSF = numel(unique(currFilts)):-1:1
  sph(iSF) = subplot(numel(unique(currFilts)),1,iSF);
  if iSF == 1
    title([' BSNR = ',num2str(starID)])
  end
  i_currF = find(currFilts == uFilts(iSF));
  ph(iSF) = scatter3(curr_sU(i_currF).*curr_sV(i_currF),...
                     curr_dl(i_currF),...
