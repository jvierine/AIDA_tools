%%function [N_all,Rayleighs_per_count] = Spec_cal_jobb_exmplCell(files)
% SPEC_CAL_JOBB_EXMPLCELL - 
...
dFiles = dir2(pwd,'*.fits','-r');
dFiles = dFiles(70:end);         
files = dFiles;      
files(979:end,:) = []; 
...
for i1 = size(gI2,1):-1:1
  for i2 = size(gI2,2):-1:1
    if ~isempty(gI2{i1,i2})
      ...
      I3{i1,i2} = gI3{i1,i2} ./ costheta{i1,i2};     
    end
  end
end
...
[N_all,qAll] = spc_sens_pdf(I2,I_sis,gfilter,filter2wl,hist_range);
...
for i1 = 1:size(I2,1)
  for i2 = 1:size(I2,2)
    if ~isempty(I2{i1,i2})
      C_samples{i2} = [C_samples{i2},I2{i1,i2}/I_sis(i1,i2)];
      Stat_C{i2}(i_samples(i2),:) = [mean(I2{i1,i2}),...
                                 median(I2{i1,i2}),...
                                 std(I2{i1,i2}),...
                                 mean(I2{i1,i2}/I_sis(i1,i2)),...
                                 median(I2{i1,i2}/I_sis(i1,i2)),...
                                 std(I2{i1,i2}/I_sis(i1,i2)),...
                                 mad(I2{i1,i2}/I_sis(i1,i2),0),...
                                 mad(I2{i1,i2}/I_sis(i1,i2),1)];
      i_samples(i2) = i_samples(i2) + 1;
    end
  end
end
Cstats = @(C) [mean(C),median(C),std(C),mad(C,0),mad(C,1),std(C)/mean(C),mad(C,1)/median(C)];
for i1 = 1:numel(C_samples)
  C_Stats(i1,:) = Cstats(C_samples{i1});
end
for i1 = numel(C_samples):-1:1
  Brigthness_per_count(i1,:) = 1./C_Stats(i1,1:2)/max(ffc_raw(:));
end

% Then to get to surface brightness (photons/m^2/s/ster) take:
for i1 = size(N_all):-1:1
  Brigthness_per_count(i1) = 1./mode(N_all{i1})/max(ffc_raw);
end
...
[N_hists0,nP1_Gaussparams0,Chi2_vals0,nNoverI0] = spc_sens_hist(I2,I_sis,gfilter,hist_range);
[N_hists,nP1_Gaussparams,Chi2_vals,nNoverI] = spc_sens_hist(I2,I_sis,gfilter,filter2wl,hist_range);