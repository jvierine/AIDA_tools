function [N_all,nP_all,Chi2_all,nNoverI] = spc_sens_hist(I_img,I_star,filter_in,filter2wl_order,hist_range)
...
for ii = 1:length(filters)
  ...
  for jj = 1:size(I_star,1)
    ...
    if ~isempty(I_img{jj,ii})
      ...
      nNoverI{ii}(jj) = numel(I_img{jj,ii})*median(I_img{jj,ii});
      %n1(jj) = length(i_f);
