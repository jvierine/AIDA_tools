function [idstarsok,stars_par] = star_int_search_man(img_in,optpar,optmode,plstars,OPTS)
...
for j = 1:length(Ivmag)
  ...
  if ( ( 1 < x0 && x0 < bx ) && ( 1 < y0 && y0 < by ) )
    ...
    [starpar,fval,exitflag] = fminsearch(@(fv) stardiff2(fv,x1,y1,starmat-bakgr2-bg3,x1(indxmax),y1(indxmax)),...
                                         startvec,fms_opts);
    if exitflag ~= 1
      [starpar,fval,exitflag] = fminsearch(@(fv) stardiff2(fv,x1,y1,starmat-bakgr2-bg3,x1(indxmax),y1(indxmax)),...
                                           starpar,fms_opts);
    ...
    if 1 || isfield(OPTS,'plotintermediates') && ...
      ...
      colorbar
      subplot(2,2,2)
      imagesc(xmin:xmax,ymin:ymax,bakgr2+bg3)
      axis xy
      colorbar
      subplot(2,2,3),imagesc(xmin:xmax,ymin:ymax,fynd)
      axis xy
      colorbar
      hold on
      ...
      hold off
      subplot(2,2,4)
      imagesc(xmin:xmax,ymin:ymax,starmat-bakgr2-bg3-fynd)
      axis xy
      colorbar
