function [identstars] = star_int_model_prime(img_in,optpar,optmodel,plstars,spp_all,OPTS)
% 
% STAR_INT_MODEL_PRIME
%
% Input:
%   img_in   - Image.
...
for iStar = 1:size(plstars,1)
  ...
  if ( ( 1 < x0 && x0 < bx ) && ( 1 < y0 && y0 < by ) )
    ...
    if ~isempty(OPTS) && isfield(OPTS,'bgtype') && strcmp(OPTS.bgtype,'complicated')
      ...
      % back_gr2 = griddata(X,Y,b,x1,y1,'v4');
      back_gr2 = gridfit(X,Y,b,x1(1,:),y1(:,1));
    ...
    subplot(spp_all{iStar}(1,1),spp_all{iStar}(1,2),spp_all{iStar}(1,3))
    imagesc(xmin:xmax,ymin:ymax,starmat),axis xy,colorbar
    cx = caxis;
    subplot(spp_all{iStar}(2,1),spp_all{iStar}(2,2),spp_all{iStar}(2,3)),
    % imagesc(xmin:xmax,ymin:ymax,back_gr2+bg3),axis xy,colorbar
    imagesc(xmin:xmax,ymin:ymax,star_intm),axis xy,colorbar
    % caxis(cx)
    subplot(spp_all{iStar}(3,1),spp_all{iStar}(3,2),spp_all{iStar}(3,3)),
    imagesc(xmin:xmax,ymin:ymax,fynd),axis xy,colorbar
    hold on
    % caxis(cx)
    ...
    hold off
    subplot(spp_all{iStar}(4,1),spp_all{iStar}(4,2),spp_all{iStar}(4,3)),
    imagesc(xmin:xmax,ymin:ymax,starmat-back_gr2-bg3-fynd),axis xy,colorbar
