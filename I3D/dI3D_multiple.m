function [V_em] = dI3D_multiple(I0_r0_dr,X,Y,Z,dt,tau,mag_ze)
...
for i1 = 1:min(size(I0_r0_dr))
  ...
  % Second try! different vertical and horisontal shape factors 
  if numel(mag_ze) == 1
    Ixyz = exp(-abs( ( ( X - x_0(end)) * cos(fi1) + ( Y - y_0 ) * sin(fi1) ).^2/dsx(end)^2 +...
                     ( (Y - ( y_0(end) - sin(mag_ze)*(Z-z_0(end)) ) ) * cos(fi1) - ( X - x_0(end) ) * sin(fi1) ).^2/dsy(end)^2 ...
                     ).^(gamma_xy/2) ...
               -abs( (Z-z_0(end)).^2 ...
                     /dsz1(end)^2.*(Z<z_0(end)) ...
                     + (Z-z_0(end)).^2 ...
                     /dsz2(end)^2.*(Z>=z_0(end))).^(gamma(end)/2));
  else
    
    Ixyz = exp(-abs( ( ( X - ( x_0(end) - sin(mag_ze(1))*sin(mag_ze(2))*(Z-z_0(end)) )) * cos(fi1) + ( Y - y_0 ) * sin(fi1) ).^2/dsx(end)^2 +...
                     ( ( Y - ( y_0(end) - sin(mag_ze(1))*cos(mag_ze(2))*(Z-z_0(end)) ) ) * cos(fi1) - ( X - x_0(end) ) * sin(fi1) ).^2/dsy(end)^2 ...
                     ).^(gamma_xy/2) ...
               -abs( (Z-z_0(end)).^2 ...
                     /dsz1(end)^2.*(Z<z_0(end)) ...
                     + (Z-z_0(end)).^2 ...
