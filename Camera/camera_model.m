...
%           1 - f*tan(theta),
%           2 - f*sin(alpha*theta),
%           3 - f(alpha*theta + (1-alpha)*tan(theta))
%           4 - f*theta
%           5 - f*tan(alpha*theta)
...
elseif optmod == -3
  ...
  switch optmod
   case 4
    % ( u^2 + w^2 )^1/2 = f*theta^alpha
    %## For pixel field-of-view to be finite alpha has to be 1.
    
    % thetas
    atan_sese1sese2 = abs( atan(((sese1).^2+(sese2).^2).^.5./(sese3)) );
    % For check-for-divide-by-zero
    sese1_p_sese2 = ((sese1).^2+(sese2).^2).^.5;
    u2 = f1*(sese1)./sese1_p_sese2.*atan_sese1sese2.^alpha;
    w2 = f2*(sese2)./sese1_p_sese2.*atan_sese1sese2.^alpha;
   ...
   case 3
    % ( u^2 + w^2 )^1/2 = f(a*theta + (1-a)*tan(theta))
    % Second term
    u1 = f1*(1-alpha)*(sese1)./(sese3);
    w1 = f2*(1-alpha)*(sese2)./(sese3);
    % thetas
    atan_sese1sese2 = atan(((sese1).^2+(sese2).^2).^.5./(sese3));
    % For check-for-divide-by-zero
    sese1_p_sese2 = ((sese1).^2+(sese2).^2).^.5;
    % First term
    u2 = f1*alpha*(sese1)./sese1_p_sese2.*atan_sese1sese2;
    w2 = f2*alpha*(sese2)./sese1_p_sese2.*atan_sese1sese2;
    % Check for divide-by-zero
    u2(sese1_p_sese2 == 0 ) = 0;
    w2(sese1_p_sese2 == 0 ) = 0;
    % Shift to center of image frame
   ...
   case 2
    ...
    u2 = f1*(sese1)./((sese1).^2+(sese2).^2).^.5.*sin(alpha*theta);
    w2 = f2*(sese2)./((sese1).^2+(sese2).^2).^.5.*sin(alpha*theta);
    % Check for divide-by-zero
    u2(sese1_p_sese2 == 0 ) = 0;
    w2(sese1_p_sese2 == 0 ) = 0;
   ...
   case 5
    ...
    u2 = f1*(sese1)./((sese1).^2+(sese2).^2).^.5.*tan(alpha*theta);
    w2 = f2*(sese2)./((sese1).^2+(sese2).^2).^.5.*tan(alpha*theta);
    % Check for divide-by-zero
    u2(sese1_p_sese2 == 0 ) = 0;
    w2(sese1_p_sese2 == 0 ) = 0;
   ...
   case 11
    ...
    w = y/by;
   case 12 
     % the generalized combination of f/a*tan(a*th), f*th, f/a*sin(a*th)
     if alpha == 0
       % thetas
       atan_sese1sese2 = abs( atan(((sese1).^2+(sese2).^2).^.5./(sese3)) );
       % For check-for-divide-by-zero
       ...
       % thetas, all of the theta
       theta = atan(((sese1).^2+(sese2).^2).^.5./(sese3));
       % For check-for-divide-by-zero we need to know if point is on
       % optical axis, then the sum of the projections on e1 and e2 are
       % zero
       sese1_p_sese2 = ((sese1).^2+(sese2).^2).^.5;
     end
     if alpha > 0
       % ( u^2 + w^2 )^1/2 = f*tan(alpha*theta)
       u2 = f1/alpha*(sese1)./((sese1).^2+(sese2).^2).^.5.*tan(alpha*theta);
       w2 = f2/alpha*(sese2)./((sese1).^2+(sese2).^2).^.5.*tan(alpha*theta);
       % Check for divide-by-zero
       u2(sese1_p_sese2 == 0 ) = 0;
       w2(sese1_p_sese2 == 0 ) = 0;
      elseif alpha < 0
       % ( u^2 + w^2 )^1/2 = f*sin(alpha*theta)
       u2 = f1/alpha*(sese1)./((sese1).^2+(sese2).^2).^.5.*sin(alpha*theta);
       w2 = f2/alpha*(sese2)./((sese1).^2+(sese2).^2).^.5.*sin(alpha*theta);
       % Check for divide-by-zero
       u2(sese1_p_sese2 == 0 ) = 0;
       w2(sese1_p_sese2 == 0 ) = 0;
     else
       % ( u^2 + w^2 )^1/2 = f*theta
       %   
       u2 = f1*(sese1)./sese1_p_sese2.*atan_sese1sese2;
       w2 = f2*(sese2)./sese1_p_sese2.*atan_sese1sese2;
       % Check for divide-by-zero
       u2(sese1_p_sese2 == 0 ) = 0;
       w2(sese1_p_sese2 == 0 ) = 0;
     end
