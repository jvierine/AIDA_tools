...
% $$$     end
   ...
   case 12 
     if alpha < 0
       % ( u^2 + w^2 )^1/2 = f/alpha*sin(theta*alpha)
       % theta = 1/alpha*asin(alpha/f*( u^2 + w^2 )^1/2)
       r = ( ( ( U-.50-du ) / f1 ).^2 + ( ( V-.50-dv ) / f2 ).^2 ).^.5;
       theta = asin( alpha*r )/alpha;
       phi = atan2((U-0.50-du)/(f1),(V-0.50-dv)/(f2));
       
     elseif alpha > 0
       % ( u^2 + w^2 )^1/2 = f/alpha*tan(theta*alpha)
       % theta = 1/alpha*atan(1/f*( u^2 + w^2 )^1/2)
       r = ( ( ( U-.50-du ) / f1 ).^2 + ( ( V-.50-dv ) / f2 ).^2 ).^.5;
       theta = (atan( alpha*r )/alpha );
       phi = atan2((U-0.50-du)/(f1),(V-0.50-dv)/(f2));
