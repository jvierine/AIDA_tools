function [xtr,ytr,ztr,tu1,tv1,tu2,tv2,minl] = aida_triangulate(img1,r1,img2,r2,cm1,cm2,PO,Opts)
% AIDA_TRIANGULATE - stereoscopic triangulation from a pair of images
% AIDA_TRIANGULATE is a function for determination of 3D positions of
...
%
% Calling:
% [xtr,ytr,ztr,tu1,tv1,tu2,tv2] = aida_triangulate(img1,r1,img2,r2,cm1,cm2,PO,OPS)
% OPS = aida_triangulate
% 
% Input:
...
while ( button ~= 2 && lower(char(button)) ~= 'm')
  ...
  if ( button == 1 || lower(char(button)) == 'l')
    ...
    if isfield(dOpts,'epipolar_lines') &&  dOpts.epipolar_lines
	...
	tv2(iPoint) = y2;
        ...
        if SubplotsNotFigs
          axes(ax1)
          hold on
          plot(x1,y1,'+','color',dOpts.pclr)
          axes(ax2)
          hold on
          plot(x2,y2,'+','color',dOpts.pclr)
        else
          hold on
          plot(x1,y1,'+','color',dOpts.pclr)
          figure(fig2)
