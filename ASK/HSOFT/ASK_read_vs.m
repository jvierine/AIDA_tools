...
% Output
%   VS - struct with meta-data for the mega-block of ASK-data. The
%   mega-block is a minutes-long (typically ~20 minutes) with
%   images taken at a fixed camera-parameters filters, frame-rate
%   etc, and is essentially the basic data-format of the ASK-system
%   even if the individual images are saved in individual files or
%   in files with images from a 2 s period. The VS-struct is a
%   primarily a global struct used in a number of files directly
%   handling ASK-data IO. Fields of user interest are:
%    .vres  - frame-rate (s), double [1 x ncams] 
%    .vlat  - latitude of observation location (deg), double scalar
%    .vlong - latitude of observation location (deg), double scalar
%    .dimx  - horizontal image sizes [#pixels], int [1 x ncams]
%    .dimy  - vertical image sizes [#pixels], int [1 x ncams]
%    .vftr  - filter identifier, cell-array with strings of wavelength
%    .vnl   - number of images in mega-block, integer [1 x ncams]
%   Users are stronlgy recommended to read but never modify values
%   in VS.
% Example:
%   % To read the meta-data for the mega-blocks from 2017-01-27
%   % starting at 20:42:39:
%   ASK_read_vs(1,...
%               ['20170127204239r1.txt';...
...
%   end
  ...
  vs.vtyp{i1} = str;
  str = fgetl(fID);
  % Reading in the dark-filenames
  iDK = findstr(str,'dark');
  if numel(iDK) < 2
    vs.vdrk{i1} = str;
  else
    vs.vdrk{i1}    = {str(1:iDK(1)+3)};
    vs.vdrk{i1}{2,1} = str((1+iDK(1)+4):end);
  end
  str = fgetl(fID);
  vs.vflt{i1} = str;
  fclose(fID);
