function varargout = ASK_images2movie(indices2show,Cams,movie_name,OPS)
...
%  figure
%  imgstacks = ASK_image_sequence(idx20170127_flickering(1):idx20170127_flickering(2),...
%                                 [1 2 3],OPS4imseq);
...
dOPS.filtertype = {'none'}; % Kernel for linear image filtering
...
vidObj = VideoWriter(movie_name); % ,'FrameRate',round(1/vs.vres));
vidObj.FrameRate = round(1/vs.vres);
[m_dir,m_name,m_ext] = fileparts(movie_name);
movie_nameF = fullfile(m_dir,[m_name,'-F',m_ext]);
vidObjF = VideoWriter(movie_nameF); % ,'FrameRate',round(1/vs.vres));
vidObjF.FrameRate = round(1/vs.vres);
...
for num = indices2show
  ...
  if length(Cams) == 1 % one camera just display!
    ...
    if dOPS.imdisplay
      ...
      avg_cIMG = avg_cIMG + cImg/subsample_factor;
      % Decorations:
  ...
  if rem(iI,subsample_factor) == 0
    imagesc(avg_cIMG);axis xy
    title(['ASK: ',ASK_dat2str(ASK_time_v(num,1),'yyyy-mm-dd HH:MM:SS.FFF')],'fontsize',16);
    currFrame = getframe(gcf);
    writeVideo(vidObjF,currFrame);
    avg_cIMG = 0;
  end
end
close(vidObj);
