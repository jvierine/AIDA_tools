...
switch radar_site(1)
 ...
 case 'E' % ESR
  ...
  % TODO: include a time-variable here, or look av vs.time of event
  % select pointing direction accordingly
  % azi = 181.0*pi/180; % Azimuth, radians % Pre 2012 summer? ASK IH
  azi = 184.5*pi/180; % Azimuth, radians % 2012-12
...
r = mean( ( (u-u0).^2 + (v-v0).^2 ).^(1/2), 'omitnan');
dr = std( ( (u-u0).^2 + (v-v0).^2 ).^(1/2), 'omitnan');
