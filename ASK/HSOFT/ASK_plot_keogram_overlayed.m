function [keos,imstack,timeV,timeIms] = ASK_plot_keogram_overlayed(timeV,keos,imstack,timeIms,OPS)
% ASK_PLOT_KEOGRAM_OVERLAYED - to plot an overlayed keogram, ASK images, 
% and keograms in one figure.
%
% CALLING:
%   OPS = ASK_plot_keogram_overlayed;
%   [keo,imstack,timeV] = ASK_plot_keogram_overlayed(timeV,keos,imstack,timeIms,OPS);
% INPUTS:
%
...
% Example:
%   % ASK-meta-data-handling for relevant mega-block
%   ASK_read_vs(1,['20170127204239r1.txt';...
%               '20170127204239r2.txt';...
%               '20170127204239r3.txt';])
%
%   % Settings for ASK-keograms and image-sequences
%   OPS4keo = ASK_keogram_overlayed;
%   OPS4keo.filtertype = {'medfilt2'};
%   OPS4keo.filterArgs = {{[3 3],'symmetric'}};
%   OPS4keo.subplot4imgs = [5,9,10];
%   OPS4keo.subplot4ASK1keo = [5,1,3];
%   OPS4keo.subplot4ASK3keo = [5,1,5];
%   OPS4keo.subplot4ASK2keo = [5,1,4];
%   OPS4keo.subplot4RGBkeo = [5,1,1];
%   OPS4keo.oneImg = 4;
%   
%   %  Indices for Time-of-interest
%   idx20170127_flickering = ASK_time2indx([2017 1 27 20 51 25;2017 1 27 20 51 52]);
%   % ASK-style Overview-keogram
%   figure('position',[300 674 940 420])
%   [keo,imstack,timeV] = ASK_keogram_overlayed(idx20170127_flickering(1),...
%                                               idx20170127_flickering(2),...
%                                               1,...
%                                               [0 0 0],...
%                                               5,...
%                                               127,...
...
dOPS.imrowDatetitle = -1;       % subplot for date-title on images-row.
...
load('data4keogram_overlayed.mat',...
     'keos','timeV','yaxis','imstack',...
     'NNN','nelem','ste','fir','las',...
     'dOPS','time_w','timeIms','shft',...
     'ran1','ran2','ran3','calib','filtStr','vs')
...
ran1 = [min(keos{1}(:)), max(keos{1}(:))];
ran2 = [min(keos{2}(:)), max(keos{2}(:))];
ran3 = [min(keos{3}(:)), max(keos{3}(:))];
...
if ~isempty(dOPS.subplot4imgs)
  ...
  % These filter-labels loaded from file produced in earlier run
  ...
  for jj = NNN:-1:1
    ...
    ASK_v_select(1,'quiet');             % Set current camera to 1
    im1 = imstack{1}(:,:,jj); % ASK_read_v(num + shft(1),[],[],[],dOPS); % Read the ASK#1 image
    ASK_v_select( 2, 'quiet');           % Cycl.
    im2 = imstack{2}(:,:,jj); % ASK_read_v(num + shft(2),[],[],[],dOPS);     % Cycl.
    ASK_v_select( 3, 'quiet');           % Cycl..
    im3 = imstack{3}(:,:,jj); % ASK_read_v(num + shft(3),[],[],[],dOPS);     % Cycl..
...
if ~isempty(dOPS.subplot4ASK1keo)
  ...
  % keogram already calibrated: PHs{3} = imagesc(timeV,yaxis,keos{1}*calib(1)/vs.vres(vs.vsel));axis xy
  PHs{3} = imagesc(timeV,yaxis,keos{1});axis xy
...
if ~isempty(dOPS.subplot4ASK2keo)
  ...
  % keogram already calibrated: PHs{4} = imagesc(timeV,yaxis,keos{2}*calib(2)/vs.vres(vs.vsel));axis xy
  PHs{4} = imagesc(timeV,yaxis,keos{2});axis xy
...
if ~isempty(dOPS.subplot4ASK3keo)
  ...
  % keogram already calibrated PHs{5} = imagesc(timeV,yaxis,keos{3}*calib(3)/vs.vres(vs.vsel));axis xy
  PHs{5} = imagesc(timeV,yaxis,keos{3});axis xy
