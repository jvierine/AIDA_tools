...
%% Here's what needs to be asserted:
%  1, that we transform the coordinates of the coordinates of the
%     slice into the local coordinate systems at station 1 and 2.
%     a, to achieve that we need to  make sure we use the trmtr
%        matrices in the .obs structs (or if there are such in the
%        stn-structs)
%     b, this certainly goes for the B-field too.
%  2, best might be to use one of the stations as the
%     central-station, then that coordinate transformation is
%     already done. Then we need to calculate XYZ-slice in that
%     coordinate system, and transform that to the other coordinate
%     system
