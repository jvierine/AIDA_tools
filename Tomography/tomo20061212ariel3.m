...
if 1
  ...
  data_dir = '/export/data/bjg1c10/ALIS/stdnames/';
  cd(data_dir)
  events = alis_event_reader('/home/bjg1c10/ALIS-ASK/ALIS-ASK-events.txt');
  ...
  load S010_2006121219320000B.acc % S010_2006121219222500B.acc
  load S04_2006121219495500T.acc
  optpB = S010_2006121219320000B([7:14 6 end]);
...
file_list = char(dFiles{1}(1,:),dFiles{2}(1,:),dFiles{3}(1,:),dFiles{6}(1,:));
...
i_z = [10 12 14]; % 100 105 110
i_x = [42 62];    % EISCAT Skibotn
i_y = [69];       % EISCAT
[alts,widths] = meshgrid([100,105,110,115],[10,15]);
...
for i2 = length(i_tb):-1:1
  ...
  for i3 = 1:length(i_stns)
    [d,h,o] = inimg(dFiles{i_stns(i3)}(i_tb(i2),:),POs(i_stns(i3)));
    stns(i3).img = d;
    if i3 == 3
  ...
  %Vem0 = tomo_start_guess1(stns(1),alt_max5577,width,XfI,YfI,ZfI);
  [Vem0,stns] = tomo_start_guess_altvar1([alts(:),widths(:)],stns,XfI,YfI,ZfI,tomo_ops);
  nr_laps = 1;
  fS = [7 5 3];
  Vem = Vem0;
  ...
  [Vem,stns(1:2)] = tomo_steps(Vem,stns(1:2),tomo_opssirt,4);
  %[Vem,stns([1,2,4])] = tomo_steps(Vem,stns([1,2,4]),tomo_opssirt,2);
  %[Vem,stns([1,2,4])] = tomo_altmaxIscaling(Vem,stns([1,2,4]),tomo_ops([1,2,4]),XfI,YfI,ZfI);
  for i_f = 2:length(fS)
    
    [xf,yf] = meshgrid(1:fS(i_f),1:fS(i_f));            
    fK = exp(-(xf-mean(xf(:))).^2/mean(xf(:)).^2-(yf-mean(yf(:))).^2/mean(yf(:))^2);
    tomo_ops(1).filterkernel = fK;
    [Vem,stns([1,2])] = tomo_steps(Vem,stns([1,2]),tomo_ops([1,2]),nr_laps);
  end
  %[Vem,stns(1)] = tomo_steps(Vem,stns(1),tomo_artops1,nr_laps);
  Vs1(:,i2,:) = Vem(:,i_x(1),:);
  Vs2(:,i2,:) = Vem(:,i_x(2),:);
