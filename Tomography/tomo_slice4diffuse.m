function [Vem,I2D,Ie2D] = tomo_slice4diffuse(stns,Energy,Ie2H,Xslice,Yslice,Zslice,M2Dto1D,U,V,I_cuts,iPeaks,X3D,Y3D,Z3D,ops)
...
% each/both/either of the cuts/stations! So, then we should be able
...
if (length(iPeaks{1}) == 1) && (length(iPeaks{2}) == 1)
  for iStn = [2, 1] % iStn - station index
...
medianI1Hor_p4SG = median(I1Hor_p4SG(:),'omitnan');
