...
if nargin < 5 || isempty(filtersizes)
  filtersizes = [7 3];
end
NR = floor(nr_imgs/2);
Df = 0;
...
for i1 = -NR:NR
  ...
  if isstruct(files)
    if isfield(files(i0+i1),'folder')
      curr_fname = fullfile(files(i0+i1).folder,files(i0+i1).name);
    else
      curr_fname = files(i0+i1).name;
    end
    d = inimg(curr_fname,PO);
  else
    d = inimg(files(i0+i1,:),PO);
  end
  Df = Df + wiener2(d - medfilt2(d,filtersizes(1)*[1,1],'symmetric'),...
                    filtersizes(2)*[1,1]);
