...
if numel( varargin{1} ) > 127^2 % prod( size(varargin{1}) ) > 127^2
  ...
  if length(filename) == 1 || all(filename == 0) % Unsure about
                                                 % this BG20140424 TOTO:check
    return
  else
    ...
    if min(cal_image(:)) < -(2^14)
      cal_image = cal_image + 2^15;
    elseif max(cal_image(:)) <= 1 % In case the peak image intensity is
...
elseif ALIS_FITS || obs.station < 15
