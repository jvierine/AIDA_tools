...
for ii = 1:min(max(size(out_I)),max_nrstars)
  ...
  [~,indx] = min((i_tmp(1:min(ii+4,length(out_j)))-uk(ii)).^2+ ...
...
for ii = 1:min(max(size(out_I)),max_nrstars)
  ...
  [~,indx] = min((out_i(ii)-uk(1:min(length(uk),30))).^2+(out_j(ii)-wk(1:min(length(uk),30))).^2);
  ids(ii) = indx;
...
for ii = 1:max(size(out_I))
  ...
  if ( rrr < 25 )
    ...
    [~,indx] = min((out_i(ii)-uk).^2+(out_j(ii)-wk).^2);
    ids(iii) = indx;
    iindids(iii) = ii;
    iii = iii+1;
