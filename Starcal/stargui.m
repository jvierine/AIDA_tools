...
prefs = personal_s_preferences();
SkMp.prefs = merge_structs(SkMp.prefs,prefs);
SkMp.optpar = optpar;
SkMp.optmod = optmod;
if optmod==2
  SkMp.oldfov = 180/pi*asin(1/2/abs(optpar(2)))/optpar(8);
...
SkMp.uic3(5,13) = uimenu(SkMp.uic3(5,1), ...
                         'Label','Make and Save pixel-(az,ze)', ...
                         'Callback','save_azze(SkMp)');
...
SkMp.uic4(2,2) = uimenu(SkMp.uic4(2,1),...
			'Label','autopick',...
			'Callback','[SkMp,starpar] = updstraut(SkMp);');
SkMp.uic4(2,3) = uimenu(SkMp.uic4(2,1),...
			'Label','man.pick',...
			'Callback','[SkMp,starpar] = updstrman(SkMp);');
...
for i1 = 1:size(SkMp.uic4,1)
  for i2 = 1:size(SkMp.uic4,2)
    try
      set(SkMp.uic4(i1,i2),'handlevisibility','callback')
    catch
