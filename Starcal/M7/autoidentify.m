...
while iS <imax && ~enough_already
  ...
  if ( ( 1 < x0 & x0 < bx ) & ( 1 < y0 & y0 < by ) )
    ...
    startvec = [x0,y0,tbild(round(y0),round(x0))-median(bakgr2(:)),1,0,1];
    ...
    % Allow the user to accept/skip/modify the identification.ginput
    try
      [xin,yin,button] = ginputc(1,...
                                 'color',ginp_clr,...
                                 'linewidth',ginp_lw,...
                                 'ShowPoints',false);
    catch
	...
	if ( varv < 3 )
	  ...
	  % startvec = [x0,y0,tbild(round(y0),round(x0))-bakgr2,1,.01,1];
          % starpar = fminsearch(@(startvec) stardiff (startvec,xStar,yStar,starmat),startvec);
	  startvec = [x0,y0,tbild(round(y0),round(x0))-bakgr2,1,0,1];
	...
	elseif ( 2 < varv & varv < 6 )
	  ...
	  starpar = [x0,y0,abs(tbild(round(y0),round(x0))-bakgr2),2,0,2];
	  
	else
	  
	  % starpar = [x0,y0,abs(tbild(round(y0),round(x0))-bakgr2),.5,0,.5];
	  starpar = [x0,y0,abs(tbild(round(y0),round(x0))-bakgr2),2^.5,0,2^.5];
	...
	identstars(idnr,9) = plstars(iS,3);   % star index
	identstars(idnr,10) = plstars(iS,4);  % star magnitude
	identstars(idnr,11) = starpar(4);     % dx - horizontal star-size in image(x)
	identstars(idnr,12) = starpar(6);     % dy - vertical star-size in image(y)
	identstars(idnr,13) = starpar(5);     % phi - rotation of stellar elipse
	clear starpar starmat fynd ;
    ...
    if ( exist( 'identstars','var' ) )
      ...
      title(tstr)
      xlabel('hit "enter" twice to finish this auto-identification.')
    end
