...
try
  ...
  disp('select another directory to save optpars in')
  new_res_dir = uigetdir;
  if new_res_dir == 0
    disp('Doing nothing, perhaps fix the path in SkMp.StarCalResDir manually?')
  else
    SkMp.StarCalResDir = uigetdir;
    SkMp = saveacc(SkMp);
  end
end
