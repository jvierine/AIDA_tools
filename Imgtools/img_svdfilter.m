...
dOPS.min_nSval = 1;  % Lowest number of singular values to test for
                    % filtered image
dOPS.max_nSval = 256;  % Largest number of singular values to test for
                       % filtered image
...
display_prog = dOPS.display(1);
display_prog2 = dOPS.display(end);
...
for i1 = min_nSval:max_nSval
  ...
  if display_prog == 1
    ...
    imagesc(Is),caxis(cx)
    set(gca,'yticklabel','','tickdir','both')
    title('I_{curr}')
    cbh232 = colorbar_labeled('I');
    hold on
...
% $$$     end
    ...
    title('img\_in - I_{curr}')
    caxis([-1 1]*mean(abs(caxis)))
    set(gca,'yticklabel','','tickdir','both')
    set(gca,'colormap',turbo)
    cbh233 = colorbar_labeled('\Delta{}I');
    set(cbh233,'colormap',turbo)
    if display_prog2 == 2
      subplot(2,3,5)
      imagesc(log10(abs(fftshift(fft2(U(:,i1)*S(i1,i1)*V(:,i1)')))))
      set(gca,'colormap',turbo)
      title('log_{10}(|fft2(dI_{curr})|)')      
      subplot(2,3,6)
      imagesc(U(:,i1)*S(i1,i1)*V(:,i1)')
      set(gca,'colormap',turbo)
      title('dI_{curr}')
      subplot(2,3,4)
    else
    ...
    xlabel('distribution of scaled residuals')
  ...
  elseif display_prog == 2 && any(i1==indx2plot)
