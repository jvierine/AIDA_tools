  ...
  for cam = 1:length(Imstacks)
    ...
    if ~isempty(Imstacks{cam})
      for num = 1:size(Imstacks{cam},3)
        ...
        Keos{cam}(:,num) = mean(interp2(Imstacks{cam}(:,:,num),U{cam},V{cam},'linear'),2,'omitnan');
        ...
        %   Keos{cam}(:,num) = mean(interp2(Imstacks{cam}(:,:,num),U{cam},V{cam},'linear',0),2,'omitnan');
        % end
      end
  ...
  for cam = 1:length(Imstacks)
    ...
    if ~isempty(Imstacks{cam})
      ...
      for num = 1:size(Imstacks{cam},3)
        ...
        % Extract and average the intensities in the fan
        Keos{cam}(:,num) = mean(interp2(Imstacks{cam}(:,:,num),U,V,'linear',0),2,'omitnan');
        % if exist('nanmean','file')
