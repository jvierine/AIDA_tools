...
if ~isempty(longlatalt)
  ...
  [~,~,uG,vG,cG1,cG2] = project_llh2img([long0lat0alt0;long0lat0alt0],long0lat0alt0,optpar,size(ImgIn(imReg(3):imReg(4),imReg(1):imReg(2),1)),gridstyle);
  u = [];
  v = [];
end
% Show image
...
if nargin > 4
  ...
  text(5,size(ImgIn,1)-10,...
       sprintf('Updated %s',datetime("now",'format','yyyyMMdd HH:mm:ss')),... % was: sprintf('Updated %4d%02d%02d %02d:%02d:%02d',floor(clock)),...
       'fontsize',14,...
       'FontWeight','bold',...
       'color','w')
end
