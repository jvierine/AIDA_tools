...
for i1 = idx2loop
  ...
  while exitflag ~= 1 && i2 <= dOPS.nrOfTries(min(i1,length(dOPS.nrOfTries)))
    ...
    % maximum:
    [vP{i1},fv(i1),exitflag] = fminsearchbnd(@(varpars) errFcnAuroralFlow(varpars,1:length(vP{i1}),vP{i1},I1,I2,sqrt(length(vP{i1})/3),fOPS),vP{i1},minP,maxP,fminops);
    i2 = i2+1;
  end
  ...
  imagesc(I2p-I2),
  cx = caxis;
  caxis([-1 1]*min(abs(cx)))
  dT(i1) = toc;
  disp('push anything, wait for a second...')
  pause(1)
