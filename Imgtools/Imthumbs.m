...
%  IMTHUMBS loads and downsamples a given list of image files and
...
for i1 = 1:size(inFiles,1)
  ...
  imfo2 = imfinfo(strtrim(namestr4load));
  if strcmp(imfo2(1).ColorType,'indexed')
    assignstr = ['Im',sprintf('%03d',i1),' = imreadindexed2rgb(''',namestr4load,''');'];
  else
  ...
  try
    ...
    dispstring = ['imthumb_load_n_show(''',sprintf(namestr4load),''',',num2str(fig_full.Number),')'];
  end
