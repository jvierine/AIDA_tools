function varargout = img_optflowV(I1,I2,OPS)
% IMG_OPTFLOWV - optical-flow displacements with intensity-scaling adjustment
%   
% Calling:
%   OPS = img_optflowV
%   [Xdisplacement,Ydisplacement,Idisplacement] = img_optflowV(I1,I2,OPS)
%   AllParameters = img_optflowV(I1,I2,OPS)
% Input:
...
dOPS.nrOfTries = 2;   % The number of times to start fminsearch if
                      % not reaching a 
dOPS.fcn_ver = 1;     % Optical-flow-function version, scalar
                      % integer: [{ 1 } | 2]
dOPS.Plotintermediates = 0; % To plot intermediate results or not
                            % defaults to not.
...
fminops = optimset('lsqnonlin');
fminops.Display = 'off'; % 'iter'
% loop over finer and finer grids
idx2loop = 1:(log2(dOPS.MaxGridSize-1)+1);
for i1 = idx2loop
% for i1 = 1:(log2(dOPS.MaxGridSize-1)+1),
  ...
  while exitflag < 1 && i2 <= dOPS.nrOfTries(min(i1,length(dOPS.nrOfTries)))
    ...
    % maximum:
    [vP{i1},fv(i1),~,exitflag] = lsqnonlin(@(varpars) resFcnAuroralFlow2(varpars,...
                                                      1:length(vP{i1}),...
                                                      vP{i1},...
                                                      I1,...
                                                      I2,...
  ...
  [I2p] = resFcnAuroralFlow2(vP{i1},1:length(vP{i1}),vP{i1},I1,I2,sqrt(length(vP{i1})/3),fOPS2);
  dT(i1) = toc;
  if dOPS.Plotintermediates
    subplot(2,2,1)
    title(i1)
    imagesc(I1)
    subplot(2,2,2)
    imagesc(I2)
    cx1 = caxis;
    title(sprintf('%3.2',i1/idx2loop(end)))
    subplot(2,2,4)
    imagesc(I2p)
    caxis(cx1)
    subplot(2,2,3)
    imagesc(I2p-I2),
    cx1 = caxis;
    caxis([-1 1]*min(abs(cx1)))
    drawnow
  end
  fprintf('done with iteration %d, out of: %d, in %f s\n',i1,idx2loop(end),dT(i1))
