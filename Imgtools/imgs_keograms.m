function [varargout] = imgs_keograms(files,locnrs,line_not_columns,optp,PO)
...
if nargin>=4 && length(optp)>9
  ff = ffs_correction2(PO.outimgsize.*[1 1],optp,optp(9));
else
  try
    ff = ffs_correction2(PO.outimgsize.*[1 1],optp,optp.mod);
  catch
    ff = 1;
  end
end
