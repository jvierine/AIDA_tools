function J = gen_susansigma(I,sigmaI,w,OPS)
...
%   INCLUDE_CENTER - [{0}|1] to include the center point or not to
%         include the center point, default is to not include it.
%   PROPORTIONAL - 
% OUTPUT:
    ...
    if OPS.include_center==0 && abs(i1-(regsize(1)+1)/2)<eps  && abs(j2-(regsize(2)+1)/2)<eps 
      ...
      WI = max(realmin,...
               exp( - abs((I-I_internal(i1:(i1-1+Iy),j2:(j2-1+Ix)))./...
                          (abs(max(1,sigmaI)).*OPS.tau)).^OPS.gamma ));
