function [varargout] = data_splinefilter(t_in,z_in,data_in,OPS)
% DATA_SPLINEFILTER - spline fitting optimum statistical filter
%  DATA_SPLINEFILTER is a filter based on 2-dimensional least square
...
%
%   [varargout] = data_splinefilter(img_in,OPS)
% Calling:
%   [Data,Datas,Measures,BestIndxs] = data_splinefilter(t_in,z_in,data_in,OPS);
%   OPS = data_splinefilter
% Input:
%   data_in - double array with input data to filter
%   OPS     - optional struct with parameters to control filtering
% Output:
%   Data      - average of the data of the best values of the
%               different statistical measures
%   Datas     - data for the best individual measures 
...
%                        disables pausing 
%               medianFilter [3], reg-size to use for prefiltering data
...
%  OPS.MaxKnots = 41;                   
%  clf                             
%  [Img_out,Imgs_out,Measures,bestIdxs] = data_splinefilter(Img_in,OPS);
...
dOPS.medianFilter = 3;    % reg-size to use for prefiltering data
                          % to fit the 2-D splines to
dOPS.W = ones(size(data_in));
...
if dOPS.medianFilter > 0,
  data2fit = medfilt2(data_in,[1,1].*dOPS.medianFilter);
else
  data2fit = data_in;
end
sz = size(data_in);
...
if dOPS.display == 1
  ...
  pcolor(t_in,z_in,data_in),shadiung flat
  title('data_{in}')
  cx = imgs_smart_caxis(0.003,data_in(:));
elseif dOPS.display == 2
  ...
  axes(sph(1))
  pcolor(t_in,z_in,data_in),shading flat,set(gca,'xticklabel','')
  cx = imgs_smart_caxis(0.003,data_in(:));
...
for i1 = min_knts:max_knts,
  ...
  knts_z = augknt(linspace(z_in(1),z_in(2),i1),order);
  knts_t = augknt(linspace(t_in(1),t_in(2),i1),order);
  % Least square spline approximant:
  sI(i1) = spap2({knts_z,knts_t},[1,1]*order,{z_in,t_in},data2fit,1./std_in.^2);
  % with the corresponding least square spline approximation image
  Is = fnval(sI(i1),{z_in,t_in});
  
  % Calculate histogram of the scaled residual:
  hI(i1,:) = histc(NoiseScaleFcn(data_in(:),max(1,Is(:))),x);
  ...
  if dOPS.display == 1
    subplot(2,3,3)
    pcolor(t_in,z_in,Is),shading flat,caxis(cx)
    hold on
    % with the corresponding node points:
    if i1 < 20
      [Kntx,Knty] = meshgrid(knts_t,knts_z);
      plot(Kntx,Knty,'k+')
    end
    hold off
    title('Img_N')
    ...
    subplot(2,3,2)
    pcolor(t_in,z_in,data_in - Is),shading flat,imgs_smart_caxis(0.005,data_in - Is)
    title('Scaled data_{in}-Data_N')
...
if nargout == 0
  ...
  for i1 = 1:length(BestIndxs),
    Imgs{i1} = fnval(sI(BestIndxs(i1)+min_knts-1),{z_in,t_in});
