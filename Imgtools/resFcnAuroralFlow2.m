function varargout = resFcnAuroralFlow2(varpars,ind4v_p,par0,I1,I2,nPoints,OPS)
% resFcnAuroralFlow - error function for auroral motion estimate
% 
% This function calculates the residuals between one
...
%
% Calling:
%  err = resFcnAuroralFlow(varpars,ind4v_p,par0,I1,I2,nPoints,OPS)
%  [I2p] = resFcnAuroralFlow(pars,varpars,par0,I1,I2,nPoints,OPS)
%  OPS = resFcnAuroralFlow
% 
% Input:
...
X = interp2(xP0,yP0,xP,1:size(I1,2),[1:size(I1,1)]','*spline');
Y = interp2(xP0,yP0,yP,1:size(I1,2),[1:size(I1,1)]','*spline');

[l2,l1] = meshgrid((0:(size(I1,2)-1)/(nPoints-1)),(0:(size(I1,1)-1)/(nPoints-1)));
l2 = l2/max(l2(:));
l1 = l1/max(l1(:));
...
% $$$   keyboard
% $$$ end
for i1 = 1:(size(xP,1)-1)
  for i2 = 1:(size(xP,2)-1)
    rAp = [xP(i1,i2);yP(i1,i2)];
    rBp = [xP(i1,i2+1);yP(i1,i2+1)];
    rCp = [xP(i1+1,i2+1);yP(i1+1,i2+1)];
    rDp = [xP(i1+1,i2);yP(i1+1,i2)];
% $$$     r_A{i1,i2} = rAp;
% $$$     r_B{i1,i2} = rBp;
% $$$     r_C{i1,i2} = rCp;
% $$$     r_D{i1,i2} = rDp;
    
    dA = abcd2dA(rAp,rBp,rCp,rDp,l1,l2);
    dI0((1:szIreg(1))+szIreg(1)*(i1-1),(1:szIreg(2))+szIreg(2)*(i2-1)) = 1./dA*prod(szIreg);
  end
end

% Do the same for the intensity scaling
Iscale = dI0.*interp2(xP0,yP0,Ip,1:size(I1,2),[1:size(I1,1)]','*spline');

% Preform the spatial image transformation and scale the intensities:
I2p = Iscale.*interp2(I1,X,Y,'*spline');
%keyboard
% Calculate the sum of square error for optimization:
% err = [I2p(isfinite(I2p(:))) - I2(isfinite(I2p(:)))];
% err = [I2p(:) - I2(:)];
err = [I2p(2:end-1,2:end-1) - I2(2:end-1,2:end-1)];
if any(~isfinite(err))
  keyboard
end
%,sum(~isfinite(I2p(:)))];
...
function dA = abcd2dA(rAp,rBp,rCp,rDp,l1,l2)
% ABCD2DA - 
%   


% $$$ dA = -(l1.*(rCp(1)-rDp(1))+(rAp(1)-rBp(1)).*(l1-1.0)).*(l2.*(rBp(2)-rCp(2))-(rAp(2)-rDp(2)).*(l2-1.0))+...
% $$$       (l2.*(rBp(1)-rCp(1))-(rAp(1)-rDp(1)).*(l2-1.0)).*(l1.*(rCp(2)-rDp(2))+(rAp(2)-rBp(2)).*(l1-1.0));
dA = abs((l1.*(rCp(1)-rDp(1))+(rAp(1)-rBp(1)).*(l1-1.0)).*(l2.*(rBp(2)-rCp(2))-(rAp(2)-rDp(2)).*(l2-1.0))-...
         (l2.*(rBp(1)-rCp(1))-(rAp(1)-rDp(1)).*(l2-1.0)).*(l1.*(rCp(2)-rDp(2))+(rAp(2)-rBp(2)).*(l1-1.0)));
